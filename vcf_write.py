...
def main():
    ...
    header = vcfpy.Header(lines=[vcfpy.HeaderLine(key="source", value="vcf_write.py") ], samples=vcfpy.SamplesInfos(["Sample1"]))
    ...
    with vcfpy.Writer.from_path(outvcf, header) as writer:
        ...
        # creating one record
        record = vcfpy.Record(
                CHROM="1", POS=1, ID=[], REF="C", ALT=[vcfpy.Substitution(type_="SNV", value="G")], QUAL=None, FILTER=["PASS"], INFO={"DP":"50"}, FORMAT=["GT","DP"],
